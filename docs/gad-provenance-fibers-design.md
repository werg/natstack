...
- The worktree state graph lives on a dedicated log (`VCS_LOG_ID`) with per-context
  heads (`ctx:{id}`, `main`) — *separate* from the agent trajectory log. The link
  is `contextId`.
...
| `turn` | turn id |
| `session` | trajectory branch head id (a *hint*, not authority — see §6.4) |
| `claim` | claim id |
...
ladder so the Hebbian loop *cannot* outrun organic signal — "we showed it"
must always count for less than "the agent edited/asserted it" (echo-chamber
guard; the exact ratio is a tuning knob, §12).
...
**touch-set**, over a *bounded* 2-hop neighborhood, ranked so that connections
...
- `idf(C) = 1 / log(2 + degree(C))` — **specificity**. A claim/file connected to
  *everything* is background hum; one connected to exactly the sessions that
...
Edit *magnitude* (lines/hunks) is **not** a weight input — it is a poor proxy for
...
*discriminating* connections rather than voluminous ones.

### 6.3 Decay basis: logical, two clocks

Wall-clock decay would only be nondeterministic if read *at query time*; the real
...
- **Historical leg** (`X ~ C`): decay in **per-anchor ordinality** — e.g. how
  many *later* edits to that file have occurred since the fiber, counted on the
...
`gad_fibers.session_id` column is a denormalized *hint* (branch head at creation)
...
- **Escape hatch for richer-than-2-hop ranking on large logs:** materialize a
  `gad_fiber_affinity` table from a *periodic server-driven pass* — the same
...
## 7. Read-time attachment: parallel, budgeted, best-effort, warmed

**Trigger:** first-touch + on-demand. On the **first** read of a `file:{path}`
within a session, attach a compact provenance block to the tool result and emit
`included` fibers. An explicit `provenance(path | claim | query)` tool (and raw
SQL) goes deeper on demand; subsequent reads of the same file do not re-attach.
The prompt tells the agent to lean on the first-touch summary and pull deeper
provenance only when a change's ramifications are unclear — not after every read.

### 7.1 Run it in parallel — it's genuinely concurrent here
...
### 7.2 Breathing room: a standalone budget + speculative warm

The budget is **decoupled from the single read's I/O latency** — the provenance
block is consumed by the model's *next generation* (seconds away), not by the
...
- **Speculative warm (primary).** On `turn.opened` — and again while the model is
  ...
  A first-touch read then resolves to a **cache hit** (O(1)), so the budget rarely
  bites. The cache is keyed by `touch_version` so a hit is only reused while the
  session's context hasn't moved on; it is disposable (drop = recompute), never
  authority.
...
`observed` (and any `included`) fibers *after* the query resolves — a file's own
read must not appear in its own attachment. The content fetch never touches
gad-store, and the DO serializes its own writes, so there is no contention; just
sequence query → attach → write.

Whether an attachment appears is now timing-dependent (warm hit / budget), but
that composes cleanly with replay determinism because **the log is the source of
truth, not the ranking**: provenance ranking is an ephemeral runtime decision
that is never replayed, and we only ever persist `included` fibers for what was
genuinely shown.

### 7.4 Attachment shape
...
The density meter is a coarse bucketing of `score`.
...
1. **Keystone + blame** — causality through `applyEdits`; `edited` fibers;
   ...
   *(Dead-table/kind deletion already landed.)*
...
3. **Density + attachment** — `provenanceForFile` (§6, capped 2-hop, `idf`/`norm`,
   turn/ordinality decay); parallel + budgeted + best-effort read-time attachment
   (§7.1–7.3); `included` fibers; `provenance()` tool; prompt guidance.
...
- Whether historical age should decay at all, or whether *any* past co-edit stays
