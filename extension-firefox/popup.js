/**
 * NatStack Panel Manager — Popup UI (Firefox)
 *
  ...
  function renderPanel(panel, depth) {
    ...
    if (panel.hasTab) {
      ...
      li.addEventListener("click", () => {
        browser.runtime.sendMessage({ type: "focusTab", panelId: panel.panelId });
      });
    } else if (panel.url) {
      ...
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        browser.runtime.sendMessage({ type: "openPanel", url: panel.url });
      });
      li.appendChild(btn);
    } else {
      const badge = document.createElement("span");
...
// Get initial status — Firefox returns a Promise from sendMessage
