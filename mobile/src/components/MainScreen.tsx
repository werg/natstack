...
import { View, Text, StyleSheet, BackHandler, Platform, Linking, Appearance, Alert } from "react-native";
...
/** Find a panel by ID in a nested tree */
function findPanelInTree(panels: Panel[], id: string): Panel | null {
  for (const panel of panels) {
    if (panel.id === id) return panel;
    const found = findPanelInTree(panel.children, id);
...
export function MainScreen() {
  ...
  const activatePanel = useCallback(
    (panelId: string) => {
      ...
      const panel = findPanelInTree(rawTreeRef.current, panelId);
      if (!panel) return;
  ...
  }, [shellClient, refreshTree, activatePanel]);
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.contentArea}>
        ...
        {webViewStack.map((entry) => (
          <WebViewErrorBoundary key={entry.panelId} panelId={entry.panelId}>
            <PanelWebView
              ref={(handle) => {
                if (handle) {
