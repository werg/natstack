 ...
 *
 * The panel API is exported here and re-exported by @natstack/panel.
 * Either package can be used to access the panel API.
 */

// Re-export zod for convenience (panels can use this instead of adding zod directly)
export { z } from "zod";
export type { ZodType, ZodObject, ZodRawShape } from "zod";

// Panel API (browser-only)
export {
  default as panel,
  type PanelAPI,
  type PanelTheme,
 ...
 * @example
 * ```ts
 * import { z, type EventSchemaMap } from "@natstack/core";
 *
 ...
 * @example
 * ```ts
 * import { z, type InferEventMap } from "@natstack/core";
 *
 * const schemas = {
...
import type * as Rpc from "./types.js";
...
> {
   ...
   * Works for all child types (panels have CDP too).
   */
