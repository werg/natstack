...
export function urlMatchesEqual(match1, match2) {
    if (isRegExp(match1) && isRegExp(match2))
        return match1.source === match2.source && match1.flags === match2.flags;
    return match1 === match2;
}
export function urlMatches(baseURL, urlString, match, webSocketUrl) {
...
export function resolveGlobToRegexPattern(baseURL, glob, webSocketUrl) {
    if (webSocketUrl)
...
export function constructURLBasedOnBaseURL(baseURL, givenURL) {
    try {
