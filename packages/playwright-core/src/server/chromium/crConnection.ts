...
export class CRConnection extends SdkObject {
  ...
  async _onMessage(message: ProtocolResponse) {
    ...
    if (session) {
      ...
      // Session not found - this is a serious issue that causes hangs
      debugLogger.log('error', `CDP message for unknown session "${sessionId}": ${JSON.stringify(message).slice(0, 200)}`);
    }
  }
...
export class CRSession extends SdkObject {
  ...
  _onMessage(object: ProtocolResponse) {
    if (object.id && this._callbacks.has(object.id)) {
      ...
      // Response with ID but no matching callback - this can happen if:
      // 1. Session was disposed before response arrived
      // 2. Response routed to wrong session
      // Log and ignore instead of asserting to avoid crashing the browser connection
      debugLogger.log('error', `Unhandled CDP response with id ${object.id}: ${JSON.stringify(object.error) || 'no error'}`);
    } else {
