...
import { createRootSdkObject } from '../instrumentation';
...
export class CRBrowser extends Browser {
  ...
  static async connect(
    parentOrEndpoint: SdkObject | string,
    transportOrOptions?: ConnectionTransport | CRBrowserConnectOptions,
    options?: BrowserOptions,
    devtools?: CRDevTools
  ): Promise<CRBrowser> {
    if (typeof parentOrEndpoint === 'string') {
      const wsEndpoint = parentOrEndpoint;
      const connectOptions = transportOrOptions as CRBrowserConnectOptions | undefined;
      const transport = await BrowserWebSocketTransport.connect(wsEndpoint, connectOptions?.transportOptions);
      const browserOptions = buildWebBrowserOptions(wsEndpoint, connectOptions);
      return CRBrowser._connect(createRootSdkObject(), transport, browserOptions, devtools);
    }
    return CRBrowser._connect(parentOrEndpoint, transportOrOptions as ConnectionTransport, options as BrowserOptions, devtools);
  }

  private static async _connect(parent: SdkObject, transport: ConnectionTransport, options: BrowserOptions, devtools?: CRDevTools): Promise<CRBrowser> {
...
function buildWebBrowserOptions(wsEndpoint: string, options: CRBrowserConnectOptions | undefined): BrowserOptions {
  const {
    transportOptions: _ignoredTransportOptions,
    browserLogsCollector,
    protocolLogger,
    artifactsDir = './playwright-artifacts',
    downloadsPath = './playwright-downloads',
    tracesDir = './playwright-traces',
    browserProcess = { kill: async () => {}, close: async () => {} },
    headful,
    ...rest
  } = options || {};
  ...
  const merged: BrowserOptions = {
    ...rest,
    name: rest.name || 'chromium',
    isChromium: rest.isChromium ?? true,
    artifactsDir,
    downloadsPath,
    tracesDir,
    headful: headful ?? true,
    browserProcess: browserProcess as any,
    protocolLogger: protocolLogger || helper.debugProtocolLogger(),
    browserLogsCollector: browserLogsCollector || new RecentLogsCollector(),
    wsEndpoint,
    originalLaunchOptions: (rest as any).originalLaunchOptions || {} as any,
  };
