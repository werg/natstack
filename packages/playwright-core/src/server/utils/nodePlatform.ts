...
// Only resolve coreDir in Node environment (not browser)
const isNode = typeof process !== 'undefined' && !!process.versions?.node;
let _coreDir: string | undefined;
function getCoreDir(): string {
  if (_coreDir === undefined) {
    _coreDir = isNode ? path.dirname(require.resolve('../../../package.json')) : '';
  }
  return _coreDir;
}
...
export const nodePlatform: Platform = {
  ...
  boxedStackPrefixes: () => {
    if (process.env.PWDEBUGIMPL)
      return [];
    return [getCoreDir(), ...boxedStackPrefixes];
  },
