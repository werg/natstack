...
export class DependencyResolver {
  ...
  private fs: FsClient;
  private depsBasePath: string;

  constructor(
    fs: FsClient,
    options: GitClientOptions,
    depsBasePath: string = "/deps"
  ) {
    this.fs = fs;
   ...
   */
  resolveDependencies(
    deps: Record<string, GitDependency | string>
  ): ResolvedDependency[] {
    return Object.entries(deps).map(([name, dep]) => {
      // Allow shorthand string format: "repo-url" or "repo-url#branch"
      const normalized: GitDependency =
        typeof dep === "string" ? this.parseShorthand(dep) : dep;

      // Determine the ref to use (priority: commit > tag > branch > 'main')
      const ref =
        normalized.commit ?? normalized.tag ?? normalized.branch ?? "main";
   ...
   */
  private parseShorthand(shorthand: string): GitDependency {
   ...
   */
  async syncDependency(dep: ResolvedDependency): Promise<{
  ...
  async syncAll(
    deps: Record<string, GitDependency | string>,
    onProgress?: (name: string, action: string) => void,
    visited: Set<string> = new Set(),
    depth: number = 0
  ): Promise<Map<string, { action: string; commit: string | null }>> {
    const MAX_DEPTH = 10; // Maximum dependency depth

    // Check depth limit
    if (depth > MAX_DEPTH) {
      throw new Error(
        `Dependency depth exceeds maximum (${MAX_DEPTH}). ` +
        `This likely indicates a circular dependency.`
      );
    }

    const resolved = this.resolveDependencies(deps);
    const results = new Map<string, { action: string; commit: string | null }>();
   ...
   */
  async isCloned(name: string): Promise<boolean> {
