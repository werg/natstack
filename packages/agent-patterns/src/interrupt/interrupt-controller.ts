...
export interface InterruptController {
   ...
   * Only available when client was provided in options (or at start() time).
   * Call this at the start of message processing.
   * Returns a promise that resolves when monitoring stops (on pause or cleanup).
   *
   * @param messageId - Override the messageId for this monitoring session (used in execution-pause publish)
   * @param onPubsubPause - Override the onPubsubPause callback for this monitoring session
   */
  startMonitoring(messageId?: string, onPubsubPause?: (reason: string) => void | Promise<void>): Promise<void>;

  /**
   * Stop monitoring without clearing pause state or handlers.
   * Use this between messages to stop the pubsub iterator while preserving
   * isPaused() state and queue wiring. Call resume() separately to reset pause state.
   */
  stopMonitoring(): void;
...
export function createInterruptController<T extends AgenticParticipantMetadata = AgenticParticipantMetadata>(
  ...
  const controller: InterruptController = {
    ...
    // Pubsub monitoring
    async startMonitoring(perCallMessageId?: string, perCallOnPubsubPause?: (reason: string) => void | Promise<void>): Promise<void> {
      if (!client) {
      ...
      // Per-call overrides take precedence over constructor options
      const effectiveMessageId = perCallMessageId ?? messageId;
      const effectiveOnPause = perCallOnPubsubPause ?? onPubsubPause;
