...
export function createWebToolsExtension(deps: WebToolsDeps): PiExtensionFactory {
  ...
  const urlCache = new Map<string, { digest: string; size: number; title: string; expiresAt: number }>();
  ...
  function urlCacheGet(url: string): { digest: string; size: number; title: string } | null {
  ...
  return (pi: PiExtensionAPI) => {
    ...
    pi.registerTool({
      ...
      execute: async (_toolCallId, params) => {
        ...
        if (cached) {
          const headSlice = await deps.rpc.call<string | null>(
            "main",
            "blobstore.getRange",
            cached.digest,
            0,
