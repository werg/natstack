...
import type { PendingExtensionApproval, PendingExtensionApprovalAction, UserlandApprovalChoice, UserlandApprovalIssuer, UserlandApprovalRequest, } from "@natstack/shared/approvals";
import { userlandApprovalRequestSchema } from "@natstack/shared/approvals";
import { ExtensionManifestError, readAndValidateExtensionManifest, } from "@natstack/shared/extensionManifest";
...
import { isBinaryEnvelope, isStreamEnvelope, type BinaryEnvelope, type BodyEnvelope, type StreamChunkEnvelope, type StreamEnvelope, } from "./wireEnvelopes.js";
import type { ExtensionHostCodeIdentityResolver, ExtensionHealth, ExtensionInvocation, InstallSpec, RegistryEntry, } from "./types.js";
import { invocationFromServiceContext } from "./types.js";
interface BuildSystemLike {
    getBuild(unitPath: string, ref?: string): Promise<{
        bundlePath: string;
        dir: string;
...
interface NotificationServiceLike {
    show(notification: Omit<NotificationPayload, "id"> & {
        id?: string;
    }): string;
}
interface UserlandApprovalGrantStoreLike {
    lookup(callerId: string, subjectId: string, issuer?: UserlandApprovalIssuer): {
        choice: string;
    } | null;
    record(principal: {
        callerId: string;
        ...
        id: string;
        label?: string;
    }, choice: string, now?: number, issuer?: UserlandApprovalIssuer): Promise<void>;
}
...
export class ExtensionHost {
    ...
    constructor(private readonly deps: ExtensionHostDeps) {
        ...
        deps.buildSystem.onPushBuild((source) => {
            this.handleSourceRebuilt(source).catch((err) => {
                console.error(`[ExtensionHost] Failed to reload rebuilt extension source ${source}:`, err instanceof Error ? err.message : String(err));
            });
        });
    }
    async startEnabled(): Promise<void> {
        for (const entry of this.registry.list()) {
            if (!entry.enabled || !entry.activeBundleKey)
                continue;
            try {
    ...
    private validateExtensionManifestAtPath(nodePath: string, unitName: string): void {
        try {
            readAndValidateExtensionManifest(path.join(nodePath, "package.json"), { unitName }, fs.readFileSync as (p: string, encoding: "utf-8") => string);
        }
        catch (err) {
            if (err instanceof ExtensionManifestError)
                throw err;
            throw new ExtensionManifestError(`Extension ${unitName} manifest validation failed: ${err instanceof Error ? err.message : String(err)}`, "MANIFEST_INTERNAL");
        }
    }
    ...
    private async handle(ctx: ServiceContext, method: string, args: unknown[]): Promise<unknown> {
        switch (method) {
            ...
            case "ready":
                return this.readyFromExtension(ctx, args[0] as {
                    methods: string[];
                    hasFetch: boolean;
                });
            case "emit":
            ...
            case "log":
                return this.logFromExtension(ctx, args[0] as UnitLogRecord["level"], args[1] as string, args[2] as Record<string, unknown> | undefined);
            ...
            case "uninstall":
                return this.uninstall(ctx, args[0] as string, args[1] as {
                    purge?: boolean;
                } | undefined);
            case "setEnabled":
    ...
    async invoke(ctx: ServiceContext, name: string, method: string, args: unknown[]): Promise<unknown> {
        ...
        const invocation: ExtensionInvocation = invocationFromServiceContext(ctx, entry.name, method, randomUUID(), this.deps.codeIdentityResolver);
        ...
        try {
            return await this.deps.extensionTransport.call(entry.name, "extension.invoke", [method,
                args,
                invocation]);
        }
        catch (error) {
            ...
            this.recordExtensionLog(entry.name, "error", wrapped.message, {
                method,
    ...
    private async ensureAvailableForInvoke(ctx: ServiceContext, name: string): Promise<RegistryEntry | null> {
        ...
        return this.runInvokeAvailability(node.name, async () => {
            const current = this.registry.get(node.name);
            if (current?.enabled && current.activeBundleKey && !this.needsBuildRefresh(current, node))
                return;
            if (current?.enabled && current.activeBundleKey) {
    ...
    private async runInvokeAvailability(name: string, work: () => Promise<void>): Promise<RegistryEntry | null> {
    ...
    async handleExtensionHttpRequest(req: IncomingMessage, res: ServerResponse, name: string, remainderPath: string, caller: {
        callerId: string;
        callerKind: string;
    }): Promise<void> {
        const entry = this.registry.get(name);
        ...
        const invocation = invocationFromServiceContext(ctx, name, method, randomUUID(), this.deps.codeIdentityResolver);
    ...
    private registerRequestBody(extensionName: string, req: IncomingMessage): StreamEnvelope | undefined {
        ...
        if (typeof contentLengthHeader === "string") {
            ...
            if (Number.isFinite(declared) && declared > EXTENSION_REQUEST_BODY_MAX_BYTES) {
                const err = new Error(`Request body exceeds the ${EXTENSION_REQUEST_BODY_MAX_BYTES}-byte extension fetch limit`);
    ...
    private async writeExtensionResponseBody(extensionName: string, res: ServerResponse, body: BodyEnvelope | string): Promise<void> {
        if (!isStreamEnvelope(body)) {
        ...
        try {
            while (true) {
                const next = await this.deps.extensionTransport.call(extensionName, "extension.fetchResponseBodyChunk", [body.id]) as StreamChunkEnvelope;
                if (next.done)
                    break;
                if (next.chunk) {
            ...
            await this.deps.extensionTransport.call(extensionName, "extension.fetchResponseBodyClose", [body.id]).catch(() => { });
        }
    }
    ...
    private readyFromExtension(ctx: ServiceContext, ready: {
        methods: string[];
        hasFetch: boolean;
    }): null {
        if (ctx.callerKind !== "extension") {
    ...
    private logFromExtension(ctx: ServiceContext, level: UnitLogRecord["level"], message: string, fields?: Record<string, unknown>): null {
        if (ctx.callerKind !== "extension") {
    ...
    private async approvalForCallerFromExtension(ctx: ServiceContext, invocationValue: unknown, reqValue: unknown): Promise<unknown> {
        if (ctx.callerKind !== "extension") {
    ...
    private async requestUserlandApprovalForCaller(invocation: ExtensionInvocation, reqValue: UserlandApprovalRequest): Promise<unknown> {
        ...
        const grant = this.deps.userlandApprovalGrantStore.lookup(invocation.userlandCaller.callerId, req.subject.id, issuer);
        ...
        if (result.kind === "choice") {
            ...
            }, req.subject, result.choice, Date.now(), issuer);
        }
        return result;
    }
    async install(ctx: ServiceContext, spec: InstallSpec): Promise<void> {
        ...
        if (!ev)
            throw new Error(`No effective version for extension ${node.name}`);
    ...
    async uninstall(ctx: ServiceContext, name: string, opts?: {
        purge?: boolean;
    }): Promise<void> {
    ...
    async update(ctx: ServiceContext, name: string): Promise<void> {
        const entry = this.registry.get(name);
        if (!entry)
            throw new Error(`Extension is not installed: ${name}`);
        const node = this.findExtensionNode(name);
    ...
    listWorkspaceUnitLogs(name: string, opts?: {
        since?: number;
        ...
        const filtered = logs.filter((record) => (opts?.since === undefined || record.timestamp >= opts.since)
    ...
    async activate(name: string): Promise<void> {
        const entry = this.registry.get(name);
        if (!entry?.activeBundleKey)
            throw new Error(`Extension has no active approved build: ${name}`);
        const build = this.deps.buildSystem.getBuildByKey?.(entry.activeBundleKey);
        if (!build) {
            ...
            if (previous?.activeBundleKey) {
                ...
                if (previous.enabled) {
                    try {
                        ...
                        this.registry.patch(node.name, {
                            status: "error",
                            lastError: `Activation failed: ${message}; rollback failed: ${rollbackErr instanceof Error ? rollbackErr.message : String(rollbackErr)}`,
                        });
                    }
                }
            }
    ...
    private currentDependencyEvs(node: ReturnType<ExtensionHost["findExtensionNode"]>): Record<string, string> {
        ...
        const visit = (depName: string): void => {
            ...
            if (depEv)
                activeDependencyEvs[depName] = depEv;
            const depNode = byName.get(depName);
            if (!depNode)
                return;
            for (const transitive of depNode.internalDeps)
                visit(transitive);
        };
    ...
    private needsBuildRefresh(entry: RegistryEntry, node: ReturnType<ExtensionHost["findExtensionNode"]>): boolean {
        const currentEv = this.deps.buildSystem.getEffectiveVersion(node.name);
        if (currentEv && entry.activeEv !== currentEv)
            return true;
        if (!shallowRecordEqual(entry.activeDependencyEvs, this.currentDependencyEvs(node)))
            return true;
        const currentExternalDeps = this.currentExternalDeps(node);
        if (!shallowRecordEqual(entry.activeExternalDeps ?? {}, currentExternalDeps))
            return true;
        if (!this.hasCurrentExtensionRuntimeAbi(entry))
            return true;
    ...
    private hasUsableActiveRuntimeDeps(entry: RegistryEntry): boolean {
        if (!entry.activeBundleKey || !entry.activeRuntimeDepsKey)
            return false;
        const build = this.deps.buildSystem.getBuildByKey?.(entry.activeBundleKey);
        if (!build)
            return false;
    ...
    private findExtensionNode(nameOrRepo: string) {
        ...
        const node = this.deps.buildSystem.getGraph().allNodes().find((candidate) => candidate.kind === "extension"
            && (candidate.name === nameOrRepo || candidate.relativePath === normalizedRepo || candidate.relativePath === nameOrRepo));
        if (!node)
            throw new Error(`Unknown extension unit: ${nameOrRepo}`);
    ...
    private async requestManagementApproval(ctx: ServiceContext, action: "install" | "uninstall" | "enable" | "disable" | "update" | "reload", name: string): Promise<void> {
        ...
        if (ctx.callerKind !== "panel" && ctx.callerKind !== "worker") {
            throw new ServiceError("extensions", action, `Extension management is not available to ${ctx.callerKind} callers`, "EACCES");
        }
        ...
        if (entry?.activeEv)
            details.push({ label: "Current EV", value: entry.activeEv });
        if (currentEv)
            details.push({ label: "Candidate EV", value: currentEv });
        const decision = await this.deps.approvalQueue.request({
            kind: "extension",
            callerId: ctx.callerId,
            callerKind: ctx.callerKind,
    ...
    private recordExtensionLog(name: string, level: UnitLogRecord["level"], message: string, fields?: Record<string, unknown>, source: UnitLogRecord["source"] = "ctx.log"): void {
...
async function readNextBodyChunk(stream: FetchRequestBodyStream): Promise<StreamChunkEnvelope> {
    ...
    if (buf.length <= STREAM_CHUNK_BYTES)
        return bufferToChunk(buf);
    stream.pending = buf;
    stream.offset = 0;
    return readNextBodyChunk(stream);
}
...
function decorateExtensionUserlandApproval(extensionName: string, req: UserlandApprovalRequest): UserlandApprovalRequest {
...
function extensionManagementAction(action: "install" | "uninstall" | "enable" | "disable" | "update" | "reload"): PendingExtensionApprovalAction {
...
function dependencyEvChanges(active: Record<string, string>, candidate: Record<string, string>): NonNullable<PendingExtensionApproval["workspaceDepChanges"]> {
...
function externalDependencyChanges(active: Record<string, string>, candidate: Record<string, string>): NonNullable<PendingExtensionApproval["externalDepChanges"]> {
    const names = [...new Set([...Object.keys(active), ...Object.keys(candidate)])].sort();
    return names
        .filter((name) => active[name] !== candidate[name])
        .map((name) => ({
...
function makeExtensionDiff(repoPath: string, previousSha: string | null | undefined, sha: string | null | undefined, push?: PendingExtensionApproval["extensionDiff"] extends infer Diff ? Diff extends {
    push?: infer Push;
} ? Push : never : never): PendingExtensionApproval["extensionDiff"] {
...
function readGitDiffStat(repoPath: string, previousSha: string | null, sha: string | null): PendingExtensionApproval["extensionDiff"] extends infer Diff ? Diff extends {
    stat?: infer Stat;
} ? Stat : never : never {
    if (!previousSha || !sha)
        return null;
...
function readGitCommit(repoPath: string, sha: string | null): PendingExtensionApproval["extensionDiff"] extends infer Diff ? Diff extends {
    commit?: infer Commit;
} ? Commit : never : never {
    if (!sha)
        return null;
