...
class MobilePanels {
    ...
    async createFromSource(source: string, options?: {
        name?: string;
    ...
    async createChildPanel(parentId: string, source: string, options?: {
        name?: string;
        contextId?: string;
        focus?: boolean;
        ref?: string;
    ...
    async createBrowserPanel(parentId: string | null, url: string, options?: {
        name?: string;
        focus?: boolean;
    }): Promise<{
        id: string;
    ...
    async createRootPanel(source: string, options?: {
        ref?: string;
    }): Promise<{
        id: string;
        title: string;
    ...
    async navigatePanel(panelId: string, source: string, options?: {
        ref?: string;
        contextId?: string;
        stateArgs?: Record<string, unknown>;
    }): Promise<{
    ...
    async getAddressOptions(source: string, ref?: string): Promise<PanelAddressOptions> {
        return getSharedPanelAddressOptions({
            ...
            ref,
            git: {
                getWorkspaceTree: () => this.deps.transport.call<{
                    children: WorkspaceNode[];
                }>("main", "git.getWorkspaceTree", []),
                findRepoForPath: (path) => this.deps.transport.call<{
                    repoPath: string;
                    relativePath: string;
                } | null>("main", "git.findRepoForPath", [path]),
                status: (repoPath) => this.deps.transport.call<PanelRepoState & {
                    repoPath: string;
    ...
    async getBrowserAddressOptions(query: string): Promise<BrowserAddressOptions> {
        return getSharedBrowserAddressOptions({
            ...
            panels: this.getTree(),
            browserData: {
                searchHistoryForAutocomplete: (searchQuery, limit) => this.invokeBrowserData<Record<string, unknown>[]>("searchHistoryForAutocomplete", [{ query: searchQuery, limit }]),
                getHistory: (historyQuery) => this.invokeBrowserData<Record<string, unknown>[]>("getHistory", [historyQuery]),
                searchBookmarks: (searchQuery) => this.invokeBrowserData<Record<string, unknown>[]>("searchBookmarks", [searchQuery]),
                getSearchEngines: () => this.invokeBrowserData<Record<string, unknown>[]>("getSearchEngines", []),
            },
        });
    }
