...
export function setCredentialConsentTransport(transport: Pick<MobileTransport, "call"> | null): void {
    configuredTransport = transport;
}
export async function launchConsentFlow(params: LaunchConsentFlowParams): Promise<ConsentCallbackParams | null> {
...
export async function completeConsent(params: CompleteConsentParams): Promise<CompleteConsentResult> {
    const transport = getCredentialConsentTransport();
    return transport.call<CompleteConsentResult>("main", "credentials.completeConsent", [params]);
}
