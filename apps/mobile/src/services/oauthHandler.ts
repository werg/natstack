...
function parsePathAndQuery(rawUrl: string, pathAndQuery: string, viaCustomScheme: boolean): ParsedCallback | null {
...
function dispatch(shellClient: ShellClient, parsed: ParsedCallback): void {
    if (parsed.viaCustomScheme) {
        ...
        // capability tokens).
        console.warn(`[oauthHandler] OAuth callback delivered via natstack:// fallback (provider=${parsed.provider}). ` +
