...
import type { ConnectCredentialRequest, StoredCredentialSummary, } from "@natstack/shared/credentials/types";
...
export interface ConnectMobileOAuthCredentialRequest extends ConnectCredentialRequest {
    providerId?: string;
    redirectUri?: string;
    callbackOrigin?: string;
    timeoutMs?: number;
}
export function buildMobileOAuthRedirectUri(providerId: string, callbackOrigin = DEFAULT_CALLBACK_ORIGIN): string {
...
export async function connectMobileOAuthCredential(shellClient: ShellClient, request: ConnectMobileOAuthCredentialRequest): Promise<StoredCredentialSummary> {
    ...
    return shellClient.transport.call<StoredCredentialSummary>("main", "credentials.connect", [{
            flow: request.flow,
...
export async function waitForMobileOAuthCode(authorizeUrl: string, expectedState: string, timeoutMs = DEFAULT_FLOW_TIMEOUT_MS): Promise<string> {
