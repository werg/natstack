...
function addWebViewEntry(entries: WebViewEntry[], nextEntry: WebViewEntry): WebViewEntry[] {
    ...
    if (nextEntries.length <= MAX_WEBVIEWS)
        return nextEntries;
...
export function MainScreen() {
    ...
    const handleWebViewUnmount = useCallback((panelId: string) => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            const request = activeChromeState.kind === "browser"
                ? shellClient.panels.getBrowserAddressOptions(query).then((options) => buildAddressAutocompleteItems({
                    kind: "browser",
                    input: query,
    ...
    useEffect(() => {
        ...
        void shellClient.panels.getAddressOptions(source, selectedBranch ?? activeChromeState?.ref)
            ...
            if (!cancelled)
                setActiveRepoState({ repoPath: source });
        });
        return () => {
            cancelled = true;
        };
    ...
    const resolveApproval = useCallback(async (approvalId: string, decision: ApprovalDecision) => {
        if (!shellClient)
            throw new Error("Shell client not available");
    ...
    const activatePanel = useCallback((panelId: string) => {
        if (!shellClient || !hostConfig)
            return;
        const panel = shellClient.panels.registry.getPanel(panelId);
        if (!panel)
            return;
        setActivePanelId(panelId);
        setWebViewStack((prev) => prev.map((entry) => entry.panelId === panelId ? { ...entry, lastActive: Date.now() } : entry));
    ...
    useEffect(() => {
        ...
        const unsubNav = shellClient.transport.onEvent("event:navigate-to-panel", (_from: string, payload: unknown) => {
        ...
        void refreshPendingApprovals().catch(() => { });
        const unsubExternal = shellClient.transport.onEvent("event:external-open:open", (_from: string, payload: unknown) => {
        ...
        const unsubNotification = shellClient.transport.onEvent("event:notification:show", (_from: string, payload: unknown) => {
        ...
        const unsubApproval = shellClient.transport.onEvent("event:shell-approval:pending-changed", (_from: string, payload: unknown) => {
            const { pending } = payload as {
                pending?: PendingApproval[];
            };
            setPendingApprovals(pending ?? []);
        });
        const unsubWorkspaceRevision = shellClient.transport.onEvent("event:workspace:revision-bumped", () => {
    ...
    const handleActiveReload = useCallback(() => {
        ...
        if (currentUrl)
            pendingHistoryIntentByUrl.current.set(canonicalHistoryKey(currentUrl), getBrowserNavigationIntentForCommand("reload-panel")!);
        ...
        switch (command) {
            ...
            case "duplicate":
                ...
                if (isBrowserPanelSource(snapshot.source)) {
                    ...
                    if (url)
                        void shellClient.panels.createBrowserPanel(null, url, { focus: true })
    ...
    const showPanelActions = useCallback((panelId = activePanelId) => {
        ...
        if (Platform.OS === "ios") {
            ...
            ActionSheetIOS.showActionSheetWithOptions({
                ...
                if (command)
                    performPanelCommand(command.id, panelId);
            });
            return;
        }
    ...
    const executeAddressAction = useCallback((action: AddressAction, mode: AddressNavigationMode = "current") => {
        ...
        if (action.type === "navigate-url") {
            ...
            if (intent)
                pendingHistoryIntentByUrl.current.set(canonicalHistoryKey(action.url), intent);
            if (mode === "external") {
                ...
                if (active && isBrowserPanelSource(getCurrentSnapshot(active).source)) {
                    setWebViewStack((prev) => prev.map((entry) => entry.panelId === activePanelId ? { ...entry, url: action.url } : entry));
        ...
        if (action.type === "search" || action.type === "keyword-search") {
            ...
            const intent = getBrowserNavigationIntentForAddressAction(action);
            if (intent)
                pendingHistoryIntentByUrl.current.set(canonicalHistoryKey(url), intent);
            if (mode === "external") {
                void Linking.openURL(url);
                return;
            }
            if (mode === "current") {
                const active = shellClient.panels.registry.getPanel(activePanelId);
                if (active && isBrowserPanelSource(getCurrentSnapshot(active).source)) {
                    setWebViewStack((prev) => prev.map((entry) => entry.panelId === activePanelId ? { ...entry, url } : entry));
                    setWebViewNavigation((prev) => ({
                        ...prev,
    ...
    const recordMobileBrowserNavigation = useCallback((panelId: string, navState: WebViewNavigation) => {
        if (!shellClient || !/^https?:\/\//i.test(navState.url))
            return;
    ...
    return (<View style={[styles.container, { backgroundColor: colors.background }]}>
      <ConnectionBar />
      <AppBar title={activePanelTitle} onMenuPress={handleMenuPress} onPanelCreated={handlePanelCreated} addressBarVisible={addressBarVisible} address={activeChromeState?.editableAddress ?? ""} metadata={activeChromeState?.kind === "panel" ? formatRepoChip(activeChromeState.repo) : null} isLoading={activeChromeState?.isLoading} canGoBack={activeChromeState?.canGoBack} canGoForward={activeChromeState?.canGoForward} onToggleAddressBar={() => setAddressBarVisible((visible) => !visible)} onBack={handleActiveBack} onForward={handleActiveForward} onReload={handleActiveReload} onStop={handleActiveStop} onNavigateAddress={handleNavigateAddress} addressSuggestions={addressSuggestions} onAddressQueryChange={setAddressQuery} onSelectAddressSuggestion={(item) => executeAddressAction(item.action)} chromeKind={activeChromeState?.kind} branches={panelAddressOptions?.branches ?? []} commits={panelAddressOptions?.commits ?? []} selectedBranch={selectedBranch ?? panelAddressOptions?.repo?.branch ?? null} selectedCommit={selectedCommit ?? panelAddressOptions?.repo?.commit ?? null} dirty={Boolean(panelAddressOptions?.repo?.dirty ?? activeChromeState?.repo?.dirty)} onSelectBranch={(branch) => {
            setSelectedBranch(branch);
            setSelectedCommit(null);
        }} onSelectCommit={setSelectedCommit} onShowActions={() => showPanelActions()}/>

      <View style={styles.contentArea}>
        {!activePanelId && (<View style={styles.placeholderContainer}>
        ...
        {loadingPanelId && loadingPanelId === activePanelId && !webViewStack.some((entry) => entry.panelId === loadingPanelId) && (<View style={styles.loadingContainer}>
        ...
        {!activePanelLeasedElsewhere && webViewStack.map((entry) => (<View key={entry.panelId} style={styles.webViewSlot} pointerEvents={entry.panelId === activePanelId ? "auto" : "none"}>
            <WebViewErrorBoundary panelId={entry.panelId} colors={{
              ...
              <PanelWebView ref={(handle) => {
                if (handle) {
                    webViewRefsMap.current.set(entry.panelId, handle);
                }
            }} panelId={entry.panelId} url={entry.url} visible={entry.panelId === activePanelId} managed={entry.managed} panelInit={entry.panelInit} externalHost={externalHost} onPanelNavigate={handlePanelNavigate} onNavigationStateChange={(navState) => {
                setWebViewNavigation((prev) => ({
                    ...prev,
            ...
            }} onTitleChange={handlePanelTitleChange} onBridgeCall={handleBridgeCall} onUnmount={handleWebViewUnmount} diagnosticsEnabled={entry.managed && hostConfig?.protocol === "http"} colors={{
                background: colors.background,
                text: colors.text,
                textSecondary: colors.textSecondary,
      ...
      <ApprovalSheet approvals={visibleApprovals} onResolve={resolveApproval} onSubmitClientConfig={submitClientConfig} onSubmitCredentialInput={submitCredentialInput} onResolveUserland={resolveUserland}/>
      <Toast />
    </View>);
}
