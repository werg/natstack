 ...
 */
import type { ParticipantMetadata, Participant, RosterUpdate, Attachment, AttachmentInput, ChannelConfig, PublishOptions, UpdateMetadataOptions, Message, PubSubMessage, LeaveReason, } from "./types.js";
import { PubSubError } from "./types.js";
import type { IncomingEvent, IncomingNewMessage, IncomingUpdateMessage, IncomingErrorMessage, IncomingMethodCallEvent, IncomingMethodResultEvent, IncomingPresenceEventWithType, IncomingExecutionPauseEvent, IncomingAgentDebugEvent, AggregatedEvent, EventStreamItem, EventStreamOptions, MethodCallHandle, MethodResultChunk, MethodResultValue, MethodDefinition, MethodAdvertisement, JsonSchema, MethodExecutionContext, } from "./protocol-types.js";
import { AgenticError } from "./protocol-types.js";
import { NewMessageSchema, UpdateMessageSchema, ErrorMessageSchema, MethodCallSchema, MethodResultSchema, ExecutionPauseSchema, } from "./protocol.js";
...
export function connectViaRpc<T extends ParticipantMetadata = ParticipantMetadata>(opts: RpcConnectOptions<T>): PubSubClient<T> {
    ...
    const callChannel = async <R = unknown>(method: string, ...args: unknown[]): Promise<R> => rpc.call<R>(await getDoTarget(), method, [...args]);
    ...
    function normalizeSenderMetadata(meta: Record<string, unknown> | undefined): {
        name?: string;
        type?: string;
        handle?: string;
    } | undefined {
    ...
    function parseIncoming(pubsubMsg: PubSubMessage): IncomingEvent | null {
        const { type: msgType, payload, attachments: msgAttachments, senderId, ts, kind, id: pubsubId, senderMetadata, } = pubsubMsg;
    ...
    function rememberReplayMessage(msg: ServerMessage): boolean {
        ...
        if (replayMessageKeys.size > MAX_REPLAY_MESSAGE_KEYS) {
            ...
            for (let i = 0; i < toRemove; i++) {
                ...
                if (value !== undefined)
                    replayMessageKeys.delete(value);
            }
        }
        return true;
    }
    function handleServerMessage(msg: ServerMessage): void {
        if (!rememberReplayMessage(msg))
            return;
        switch (msg.kind) {
            case "ready": {
            ...
            case "ephemeral": {
                ...
                if (msg.type === "method-cancel" && msg.kind !== "replay") {
                    const cancelPayload = msg.payload as {
                        callId?: string;
                    } | undefined;
    ...
    async function handleMethodCallExec(event: IncomingMethodCallEvent): Promise<void> {
        ...
        const ctx: MethodExecutionContext = {
            ...
            streamWithAttachments: async (content: unknown, attachments: AttachmentInput[], streamOpts?: {
                contentType?: string;
            }) => {
                await callChannel("publish", pid, "method-result", {
                    callId: event.callId,
            ...
            resultWithAttachments: <R>(content: R, attachments: AttachmentInput[], resultOpts?: {
                contentType?: string;
            }) => ({
                content,
                attachments,
                ...
                if (msg.id > lastSeenSeq + 1) {
                    ...
                    if (gap <= MAX_GAP_SIZE) {
                        ...
                        callChannel<Array<{
                            ...
                            senderMetadata?: Record<string, unknown>;
                            attachments?: unknown[];
                        }>>("getEventRange", lastSeenSeq, msg.id - 1).then(events => {
    ...
    if (methodAdvertisements)
        subscribeMetadata["methods"] = methodAdvertisements;
    // Heartbeat to prevent stale participant eviction
    const TOUCH_INTERVAL_MS = 60000; // 1 minute
    ...
    const unregisterColdRecover = opts.recoveryCoordinator?.registerColdRecoverHandler(`pubsub:${channel}:${pid}`, attemptResubscription);
    ...
    async function ready(signal?: AbortSignal): Promise<void> {
        ...
        if (!signal)
            return readyPromise;
        if (signal.aborted)
            throw new PubSubError("ready aborted", "connection");
    ...
    async function publish<P>(type: string, payload: P, publishOptions: PublishOptions = {}): Promise<number | undefined> {
        if (closed)
            throw new PubSubError("not connected", "connection");
        const { persist = true, attachments, idempotencyKey } = publishOptions;
        const result = await callChannel<{
            id?: number;
        }>("publish", pid, type, payload, {
            persist,
            ref: undefined,
    ...
    async function updateMetadata(newMetadata: Partial<T>, updateOptions: UpdateMetadataOptions = {}): Promise<void> {
    ...
    async function sendMessage(content: string, sendOptions?: {
        replyTo?: string;
        persist?: boolean;
        attachments?: AttachmentInput[];
        contentType?: string;
        ...
        if (sendOptions?.replyTo)
            messagePayload["replyTo"] = sendOptions.replyTo;
        if (sendOptions?.contentType)
            messagePayload["contentType"] = sendOptions.contentType;
        if (sendOptions?.at)
            messagePayload["at"] = sendOptions.at;
        if (sendOptions?.metadata)
            messagePayload["metadata"] = sendOptions.metadata;
    ...
    async function updateMessage(id: string, content: string, updateOptions?: {
        complete?: boolean;
        persist?: boolean;
        attachments?: AttachmentInput[];
        contentType?: string;
        ...
        if (updateOptions?.complete !== undefined)
            payload["complete"] = updateOptions.complete;
        if (updateOptions?.contentType)
            payload["contentType"] = updateOptions.contentType;
        return await publish("update-message", payload, {
            persist: updateOptions?.persist ?? true,
            attachments: updateOptions?.attachments,
        });
    }
    async function completeMessage(id: string, options?: {
        idempotencyKey?: string;
    }): Promise<number | undefined> {
    ...
    function callMethod(providerId: string, methodName: string, args?: unknown, callOptions?: {
        signal?: AbortSignal;
    }): MethodCallHandle {
        ...
        if (callOptions?.signal) {
            if (callOptions.signal.aborted) {
                ...
                result.then(() => callOptions.signal?.removeEventListener("abort", abort), () => callOptions.signal?.removeEventListener("abort", abort));
            }
        }
        if (!state.complete) {
    ...
    function events(evtOptions?: EventStreamOptions): AsyncIterableIterator<EventStreamItem> {
        ...
        return (async function* () {
            if (includeReplay && replayMode !== "skip") {
                const replaySeed: EventStreamItem[] = replayMode === "stream" ? streamReplayEvents : aggregatedReplay;
                for (const item of replaySeed) {
                    if (isIncomingEvent(item)) {
                        if (!includeEphemeral && item.kind === "ephemeral")
                            continue;
                    }
                    yield item;
                }
            ...
            for await (const event of source) {
                if (!includeEphemeral && event.kind === "ephemeral")
                    continue;
                yield event;
            }
        })();
