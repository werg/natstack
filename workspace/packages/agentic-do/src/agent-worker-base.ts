...
import type { Attachment, ChannelEvent, ParticipantDescriptor, TurnInput, UnsubscribeResult, } from "@natstack/harness/types";
import { isClientParticipantType } from "@natstack/pubsub";
import { PiRunner, type ChannelToolMethod, type NatStackScopedUiContext, type AskUserParams, type ApprovalLevel, type ThinkingLevel, type SystemPromptMode, DispatchedError, } from "@natstack/harness";
...
import type { RpcCallOptions } from "@natstack/rpc";
import { DOIdentity } from "./identity.js";
import { SubscriptionManager } from "./subscription-manager.js";
import { DispatchedCallStore, type DispatchedCall, type DispatchedCallKind, } from "./dispatched-call-store.js";
...
type AgentAbortReason = "channel-unsubscribe" | "participant-method-dispatch" | "ask-user-dispatch" | "ui-prompt-dispatch" | "interrupt-all" | "interrupt-channel";
...
export abstract class AgentWorkerBase extends DurableObjectBase {
    ...
    constructor(ctx: DurableObjectContext, env: unknown) {
        super(ctx, env);
        const lazyRpc = {
            call: <T = unknown>(targetId: string, method: string, args: unknown[], options?: RpcCallOptions): Promise<T> => {
                return this.rpc.call<T>(targetId, method, args, options);
            },
            streamCall: (targetId: string, method: string, args: unknown[], options?: {
                signal?: AbortSignal;
            }): Promise<Response> => {
        ...
        this.subscriptions = new SubscriptionManager(this.sql, (channelId) => new ChannelClient(lazyRpc, channelId), this.identity);
    ...
    private getApiKeyForChannel(channelId: string): () => Promise<string> {
        ...
        return async () => {
            ...
            if (!credential) {
                ...
                throw new Error(`No URL-bound model credential is configured for model provider: ${providerId}`);
            }
    ...
    protected getModelCredentialTokenClaims(_providerId: string, _credential: ModelCredentialSummary): Record<string, unknown> {
        return {};
    }
    protected async handleModelCredentialMethodCall(methodName: string, args: unknown): Promise<{
        result: unknown;
        isError?: boolean;
    } | null> {
        switch (methodName) {
    ...
    private getModelCredentialOAuthConfig(providerId: string): {
        flow: ModelCredentialOAuthConfig & {
            type: "oauth2-auth-code-pkce";
        };
    ...
    private async connectModelCredentialOAuth(args: ConnectModelCredentialOAuthArgs): Promise<ModelCredentialSummary> {
        ...
        return this.rpc.call<ModelCredentialSummary>("main", "credentials.connect", [browserHandoffCallerId
                ? {
                    spec,
                    handoffTarget: {
    ...
    private installUrlBoundModelFetchProxy(modelBaseUrl: string): void {
        ...
        if (globals.__natstackModelFetchProxyInstalled)
            return;
    ...
    protected setApprovalLevel(channelId: string, level: ApprovalLevel): void {
        ...
        if (entry)
            entry.runner.setApprovalLevel(level);
    }
    protected shouldProcess(event: ChannelEvent): boolean {
        ...
        if (!isClientParticipantType(senderType))
            return false;
        return true;
    }
    ...
    protected getRunnerPromptConfig(channelId: string): {
        ...
        const systemPromptMode = rawMode === "append" || rawMode === "replace-natstack" || rawMode === "replace"
            ? rawMode
            : undefined;
        return {
    ...
    async subscribeChannel(opts: {
        ...
        if (!this._fsContextBound.has(opts.contextId)) {
            try {
                ...
                console.warn(`[AgentWorkerBase] fs.bindContext failed for contextId=${opts.contextId}:`, err);
            }
        }
    ...
    private getDeliveryCursor(channelId: string): number {
        const cursor = this.sql.exec(`SELECT last_delivered_seq FROM delivery_cursor WHERE channel_id = ?`, channelId).toArray();
    ...
    private advanceDeliveryCursor(channelId: string, seq: number): void {
        this.sql.exec(`INSERT OR REPLACE INTO delivery_cursor (channel_id, last_delivered_seq) VALUES (?, ?)`, channelId, seq);
    }
    private async repairGap(channelId: string, lastSeq: number, eventId: number): Promise<void> {
        ...
        try {
            ...
            if (!missed || !Array.isArray(missed))
                return;
    ...
    private async dispatchChannelEvent(channelId: string, event: ChannelEvent, opts?: {
        ...
        if (event.type === "method-result") {
            ...
            if (payload?.["complete"] === false) {
                ...
                await this.onCallResult(callId, payload["content"], payload["isError"] === true);
            }
            return;
        }
    ...
    protected async getOrCreateRunner(channelId: string): Promise<PiRunner> {
        ...
        const msgRows = this.sql.exec(`SELECT content FROM pi_messages WHERE channel_id = ? ORDER BY idx`, channelId).toArray();
        if (msgRows.length > 0) {
            try {
            ...
            const sessionRow = this.sql.exec(`SELECT messages_blob FROM pi_sessions WHERE channel_id = ?`, channelId).toArray();
            if (sessionRow.length > 0 && sessionRow[0]!["messages_blob"]) {
                try {
                    ...
                    for (let i = 0; i < initialMessages.length; i++) {
                        this.sql.exec(`INSERT INTO pi_messages (channel_id, idx, content) VALUES (?, ?, ?)`, channelId, i, JSON.stringify(initialMessages[i]));
                    }
                    this.sql.exec(`DELETE FROM pi_sessions WHERE channel_id = ?`, channelId);
                }
                catch (err) {
        ...
        const runner = new PiRunner({
            rpc: {
                call: <T = unknown>(target: string, method: string, args: unknown[], options?: RpcCallOptions): Promise<T> => this.rpc.call<T>(target, method, args, options),
                streamCall: (target, method, args, options) => this.rpc.streamCall(target, method, args, options),
            },
            fs: this.fs,
            uiCallbacks: this.buildUICallbacks(channelId),
            rosterCallback: () => this.buildRoster(channelId),
            callMethodCallback: (toolCallId, handle, method, args, signal, onStreamUpdate) => this.invokeChannelMethod(channelId, toolCallId, handle, method, args, signal, onStreamUpdate),
            askUserCallback: (toolCallId, params, signal) => this.askUser(channelId, toolCallId, params, signal),
            model: this.getModel(),
            ...
            hasCredentialForOrigin: async (originUrl) => {
                try {
                    ...
                    const c = await this.rpc.call<{
                        id: string;
                    } | null>("main", "credentials.resolveCredential", [{ url: probeUrl }]);
                    return c !== null;
                }
                catch {
                    return false;
        ...
        runner.subscribe((event) => {
            ...
            if (last.stopReason === "aborted") {
                ...
                this.abortContexts.delete(channelId);
                console.log(`[AgentWorkerBase] Agent turn aborted on channel=${channelId}: ` +
    ...
    private loadMessages(channelId: string): AgentMessage[] {
        const rows = this.sql.exec(`SELECT content FROM pi_messages WHERE channel_id = ? ORDER BY idx`, channelId).toArray();
    ...
    private async appendAgentContext(channelId: string, event: ChannelEvent): Promise<void> {
        ...
        const entry = this.runners.get(channelId);
        if (entry)
            entry.runner.replaceHistory(nextMessages);
    }
    private async saveMessages(channelId: string, messages: AgentMessage[]): Promise<void> {
        ...
        for (let i = 0; i < messages.length; i++) {
            this.sql.exec(`INSERT INTO pi_messages (channel_id, idx, content) VALUES (?, ?, ?)`, channelId, i, JSON.stringify(messages[i]));
        }
    }
    private recordModelCredentialInterruption(channelId: string, providerId: string, modelBaseUrl: string): void {
        const messages = this.loadMessages(channelId);
        this.sql.exec(`INSERT OR REPLACE INTO model_credential_interruptions
        (channel_id, provider_id, model_base_url, resume_count, created_at)
       VALUES (?, ?, ?, ?, ?)`, channelId, providerId, modelBaseUrl, messages.length, Date.now());
    }
    private getModelCredentialInterruption(channelId: string, providerId: string, modelBaseUrl?: string): ModelCredentialInterruption | null {
        const rows = this.sql.exec(`SELECT provider_id, model_base_url, resume_count, created_at
       FROM model_credential_interruptions
       WHERE channel_id = ? AND provider_id = ?`, channelId, providerId).toArray();
        if (rows.length === 0)
            return null;
        const row = rows[0]!;
    ...
    private clearModelCredentialInterruption(channelId: string, providerId: string): void {
        this.sql.exec(`DELETE FROM model_credential_interruptions WHERE channel_id = ? AND provider_id = ?`, channelId, providerId);
    }
    ...
    private recordAbort(channelId: string, reason: AgentAbortReason, detail?: string): void {
        ...
        console.log(`[AgentWorkerBase] Agent abort requested on channel=${channelId}: ` +
            `reason=${reason}${detail ? ` detail=${detail}` : ""}`);
    }
    private async abortAgentForReason(channelId: string, reason: AgentAbortReason, detail?: string): Promise<void> {
    ...
    private async recoverDispatchesForChannel(channelId: string): Promise<void> {
        if (this.recoveredChannels.has(channelId))
            return;
        const messages = this.loadMessages(channelId);
        ...
        for (const breadcrumb of deferred) {
            ...
            try {
                if (claimed.abandonedReason) {
                    ...
                    await this.applyResult(claimed, decodeBufferedDispatchResult(claimed.pendingResultJson), claimed.pendingIsError ?? false);
                }
                else {
    ...
    protected getOrCreateDispatcher(channelId: string, runner: PiRunner, projector: ContentBlockProjector): TurnDispatcher {
    ...
    private async invokeChannelMethod(channelId: string, toolCallId: string, participantHandle: string, method: string, args: unknown, signal: AbortSignal | undefined, onStreamUpdate?: (content: unknown) => void): Promise<AgentToolResult<any>> {
        if (signal?.aborted)
        ...
        if (gadToolCallId != null)
            this.gadToolCallByDispatch.set(callId, gadToolCallId);
        if (onStreamUpdate)
            this.streamCallbacks.set(callId, onStreamUpdate);
    ...
    private async beginGadChannelToolCall(channelId: string, piToolCallId: string, participantHandle: string, method: string, args: unknown): Promise<number | null> {
        try {
    ...
    private async completeGadChannelToolCall(callId: string, summary: string): Promise<void> {
        ...
        try {
            ...
            const blob = await this.rpc.call<{
                digest: string;
                size: number;
            }>("main", "blobstore.putText", [summary]);
    ...
    private async askUser(channelId: string, toolCallId: string, params: AskUserParams, signal: AbortSignal | undefined): Promise<AgentToolResult<any>> {
        if (signal?.aborted)
            throw new Error("aborted");
        const callerId = this.subscriptions.getParticipantId(channelId);
        if (!callerId)
    ...
    private buildUICallbacks(channelId: string): NatStackScopedUiContext {
        return {
            selectForTool: async (toolCallId, title, options, opts) => this.dispatchUiPrompt(channelId, toolCallId, "select", { title, options }, opts?.signal) as Promise<string | undefined>,
            confirmForTool: async (toolCallId, title, message, opts, meta) => this.dispatchUiPrompt(channelId, toolCallId, "confirm", { title, message }, opts?.signal, meta) as Promise<boolean>,
            inputForTool: async (toolCallId, title, placeholder, opts) => this.dispatchUiPrompt(channelId, toolCallId, "input", { title, placeholder }, opts?.signal) as Promise<string | undefined>,
            editorForTool: async (toolCallId, title, prefill) => this.dispatchUiPrompt(channelId, toolCallId, "editor", { title, prefill }, undefined) as Promise<string | undefined>,
            notify: (message, type) => {
                const participantId = this.subscriptions.getParticipantId(channelId);
                if (!participantId)
                    return;
    ...
    private async emitModelCredentialRequiredCard(channelId: string, providerId: string, modelBaseUrl: string): Promise<void> {
        ...
        if (this.credentialPromptCardsEmitted.has(key))
            return;
    ...
    async onCallResult(callId: string, result: unknown, isError: boolean): Promise<void> {
        ...
        if (!breadcrumb)
            return;
        await this.completeGadChannelToolCall(callId, `${isError ? "error: " : ""}${summarizeUnknownResult(result)}`);
    ...
    private async applyResult(breadcrumb: DispatchedCall, result: unknown, isError: boolean): Promise<void> {
        ...
        if (breadcrumb.kind === "approval") {
            if (isError || result === false || result === "deny") {
                ...
                try {
                    execResult = await runner.executeToolDirect(breadcrumb.toolName ?? toolNameFromMessages(messages, breadcrumb.toolCallId), breadcrumb.toolCallId, breadcrumb.paramsJson ? JSON.parse(breadcrumb.paramsJson) : {});
                }
                catch (err) {
                    messages = replaceToolResultMessage(messages, breadcrumb.toolCallId, {
                        role: "toolResult",
    ...
    private async finishDispatchResolution(breadcrumb: DispatchedCall, messages: AgentMessage[], continueWhenClear: boolean): Promise<void> {
        await this.ensureChannelContext(breadcrumb.channelId);
        const runner = this.runners.get(breadcrumb.channelId)!.runner;
        ...
        const interruptedAfterDispatch = (this.lastUserInterruptAt.get(breadcrumb.channelId) ?? 0) >= breadcrumb.createdAt;
    ...
    private async finalizeAbandonedDispatch(breadcrumb: DispatchedCall, messages: AgentMessage[]): Promise<void> {
    ...
    private async dispatchUiPrompt(channelId: string, toolCallId: string, kind: "select" | "confirm" | "input" | "editor", params: Record<string, unknown>, signal?: AbortSignal, meta?: {
        toolName?: string;
        toolInput?: unknown;
        ...
        if (!panel)
            throw new Error(`No panel participant in channel ${channelId}`);
        const callId = crypto.randomUUID();
        ...
        this.dispatches.store({
            ...
            paramsJson: breadcrumbKind === "approval"
    ...
    private async sendDispatchCancel(channelId: string, callId: string, reason: "user-superseded" | "worker-restart" | "user-interrupted"): Promise<void> {
        const participantId = this.subscriptions.getParticipantId(channelId);
        if (!participantId)
            return;
        await this.createChannelClient(channelId).sendEphemeralEvent(participantId, "natstack-dispatch-cancel", { callId, reason });
    }
    private async notifyDispatchesInterrupted(channelId: string): Promise<void> {
        ...
        for (const breadcrumb of pending) {
            try {
                ...
                console.warn(`[AgentWorkerBase] Failed to cancel dispatch ${breadcrumb.callId} on interrupt:`, err);
            }
        }
    }
    ...
    async onChannelEvent(channelId: string, event: ChannelEvent, opts?: {
        mode?: "auto" | "sequential";
    }): Promise<void> {
    ...
    private async resizeAttachments(channelId: string, attachments: Attachment[] | undefined): Promise<ImageContent[] | undefined> {
        ...
        for (const att of attachments) {
            ...
            try {
                ...
                const resized = await this.rpc.call<{
                    ...
                    mimeType: string;
                    wasResized: boolean;
                }>("main", "extensions.invoke", [IMAGE_SERVICE_EXTENSION,
                    "resize",
                ...
                console.warn(`[AgentWorkerBase] image-service.resize failed for channel=${channelId}; passing original:`, err);
    ...
    async onMethodCall(_channelId: string, _callId: string, _methodName: string, _args: unknown): Promise<{
        result: unknown;
        isError?: boolean;
    }> {
        return { result: { error: "not implemented" }, isError: true };
    }
    protected async resumeAfterModelCredentialConnected(channelId: string, opts?: {
        providerId?: string;
        modelBaseUrl?: string;
    }): Promise<boolean> {
        await this.ensureChannelContext(channelId);
     ...
     */
    async postClone(parentObjectKey: string, newChannelId: string, oldChannelId: string, forkAtMessageIndex: number | null): Promise<void> {
        this.sql.exec(`INSERT OR REPLACE INTO state (key, value) VALUES ('__objectKey', ?)`, this.objectKey);
        ...
        const hasPiMessages = (this.sql.exec(`SELECT COUNT(*) as cnt FROM pi_messages WHERE channel_id = ?`, oldChannelId).toArray()[0]?.["cnt"] as number ?? 0) > 0;
        if (hasPiMessages) {
            // Normalized path: rename channel via UPDATE, trim via DELETE.
            this.sql.exec(`UPDATE pi_messages SET channel_id = ? WHERE channel_id = ?`, newChannelId, oldChannelId);
            if (forkAtMessageIndex != null) {
                this.sql.exec(`DELETE FROM pi_messages WHERE channel_id = ? AND idx >= ?`, newChannelId, forkAtMessageIndex);
            }
        }
        else {
            // Legacy blob path: migrate to pi_messages during fork.
            const parentSession = this.sql.exec(`SELECT messages_blob FROM pi_sessions WHERE channel_id = ?`, oldChannelId).toArray();
            if (parentSession.length > 0) {
                try {
                    let messages = JSON.parse(parentSession[0]!["messages_blob"] as string) as AgentMessage[];
                    if (forkAtMessageIndex != null)
                        messages = messages.slice(0, forkAtMessageIndex);
                    for (let i = 0; i < messages.length; i++) {
                        this.sql.exec(`INSERT INTO pi_messages (channel_id, idx, content) VALUES (?, ?, ?)`, newChannelId, i, JSON.stringify(messages[i]));
                    }
        ...
        const subRow = this.sql.exec(`SELECT context_id, config FROM subscriptions WHERE channel_id = ?`, oldChannelId).toArray();
    ...
    protected async onPostClone(_parentObjectKey: string, _newChannelId: string, _oldChannelId: string, _forkAtMessageIndex: number | null): Promise<void> {
        // Default: no-op
    }
    // ── Fetch override ───────────────────────────────────────────────────────
    override async fetch(request: Request): Promise<Response> {
        ...
        if (segments.length >= 1 && !(this as unknown as {
            _objectKey?: string;
        })._objectKey) {
            (this as unknown as {
                _objectKey?: string;
            })._objectKey = decodeURIComponent(segments[0]!);
        }
        ...
        if (method === "__event") {
            ...
            const [event, payload, fromId] = args as [
                string,
                unknown,
                string | undefined
            ];
    ...
    override async getState(): Promise<Record<string, unknown>> {
        ...
        const piMessages = this.sql.exec(`SELECT channel_id, idx, LENGTH(content) as content_len FROM pi_messages`).toArray();
...
function makeDispatchPlaceholder(toolCallId: string, callId: string, kind: DispatchedCallKind): AgentToolResult<any> {
    return {
...
function hasToolResultMessage(messages: AgentMessage[], toolCallId: string): boolean {
    return messages.some((message) => ((message as {
        role?: string;
        toolCallId?: string;
    }).role === "toolResult" &&
        (message as {
...
function toolNameFromMessages(messages: AgentMessage[], toolCallId: string): string {
    const match = messages.find((message) => ((message as {
        role?: string;
        toolCallId?: string;
    }).role === "toolResult" &&
        (message as {
            toolCallId?: string;
        }).toolCallId === toolCallId)) as {
        toolName?: string;
    } | undefined;
    return match?.toolName ?? "unknown";
}
function replaceToolResultMessage(messages: AgentMessage[], toolCallId: string, replacement: AgentMessage): AgentMessage[] {
    const index = messages.findIndex((message) => ((message as {
        role?: string;
        toolCallId?: string;
    }).role === "toolResult" &&
        (message as {
