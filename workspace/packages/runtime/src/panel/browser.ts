...
function getRpc(): RpcBridge {
    if (!_rpc)
        throw new Error("Browser bridge not initialized");
    return _rpc;
}
/**
 ...
 */
export async function createBrowserPanel(url: string, options?: {
    name?: string;
    focus?: boolean;
}): Promise<BrowserHandle> {
    const result = await shell.createBrowserPanel(url, options) as {
        id: string;
        title: string;
    };
 ...
 */
export function onChildCreated(handler: (info: {
    childId: string;
    url: string;
}) => void): () => void {
    ...
    return () => { for (const unsub of unsubs)
        unsub(); };
}
/**
 ...
 */
export async function openPanel(source: string, options?: {
    name?: string;
    focus?: boolean;
    ...
    return {
        ...
        async page() {
            const require = (globalThis as Record<string, unknown>)["__natstackRequire__"] as ((id: string) => {
                BrowserImpl: {
                    connect(ws: string, opts: object): Promise<any>;
                };
            }) | undefined;
            if (!require) {
