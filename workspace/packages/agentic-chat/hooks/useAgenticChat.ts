...
export function useAgenticChat({ config, channelName, channelConfig, contextId, metadata = { name: "Chat Panel", type: "panel", handle: "user" }, tools, actions, theme = "dark", pendingAgentInfos, initialPrompt, sandbox, initialActionBarFile, initialActionBarProps, initialActionBarMaxHeight, onActionBarFileChange, }: UseAgenticChatOptions): {
    ...
    useEffect(() => {
        ...
        const onUnload = () => { if (mgr.isDirty)
            mgr.persist().catch((err) => console.warn("[Chat] Scope persist on unload failed:", err)); };
        const onHidden = () => { if (document.hidden && mgr.isDirty)
    ...
    const clearActionBar = useCallback(async ({ persistStateArgs = true, idempotencyKey, }: {
        persistStateArgs?: boolean;
        idempotencyKey?: string;
    } = {}) => {
...
Use package imports available to inline_ui plus relative imports for local helper files.`,
                ...
                if (hr && (hr.restored.length || hr.lost.length || hr.partial.length)) {
                    const parts: string[] = [];
                    if (hr.restored.length)
                        parts.push(`Restored: [${hr.restored.join(", ")}]`);
                    if (hr.partial.length)
                        parts.push(`Partially restored (some properties lost): [${hr.partial.join(", ")}]`);
                    if (hr.lost.length)
                        parts.push(`Lost (must be re-created): [${hr.lost.join(", ")}]`);
