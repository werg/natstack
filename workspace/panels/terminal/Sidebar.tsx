...
import { ChevronDownIcon, ChevronRightIcon, MagnifyingGlassIcon, PlusIcon } from "@radix-ui/react-icons";
...
export function Sidebar(props: {
  ...
  if (props.collapsed) {
    return (
      <Flex direction="column" align="center" width="3rem" p="2" gap="2" style={{ borderRight: "1px solid var(--gray-5)", background: "var(--gray-1)" }}>
        <Button size="1" variant="ghost" onClick={() => props.onCollapsedChange(false)} aria-label="Show terminal sidebar">›</Button>
        <Button
          size="1"
          variant="soft"
  ...
  return (
    <Flex direction="column" width="17rem" p="3" gap="3" style={{ borderRight: "1px solid var(--gray-5)", background: "var(--gray-1)" }}>
      <Flex align="center" gap="2">
        ...
        <Button size="2" variant="ghost" onClick={() => props.onCollapsedChange(true)} aria-label="Hide terminal sidebar">‹</Button>
      </Flex>
      {props.newTabPending ? (
        <Text size="1" color="gray" role="status" aria-live="polite">{props.newTabPendingLabel ?? "Starting terminal..."}</Text>
      ) : null}
        ...
        {groups.map((group) => {
          ...
          return (
            <div key={group.name}>
              ...
              >
                ...
                <Text size="1" color="gray" style={{ textTransform: "uppercase", flex: 1 }}>{group.name}</Text>
                <Badge size="1" variant="soft" color="gray">{group.rows.length}</Badge>
              </button>
              {open ? group.rows.map((row) => {
                const active = props.activeTabId === row.tab.tabId && row.tab.focusedSessionId === row.sessionId;
                return (
          <div
            role="button"
            tabIndex={0}
          ...
          >
            <Flex align="center" gap="2" minWidth="0">
              ...
              <Text size="2" weight="medium" truncate style={{ minWidth: 0 }}>{row.title}</Text>
              {row.branch ? <Text size="1" color="gray" truncate style={{ minWidth: 0, flex: "0 1 auto" }}>{row.branch}</Text> : null}
              {row.cwdBasename ? <Text size="1" color="gray" truncate style={{ minWidth: 0, flex: "0 1 auto" }}>{row.cwdBasename}</Text> : null}
              {!active && row.unread > 0 ? <Badge size="1" color="amber" variant="soft">{row.unread}</Badge> : null}
            </Flex>
            <Flex align="center" justify="between" gap="2" mt="1">
              ...
              <Flex gap="1" flexShrink="0">
                ...
                {row.extraPortCount > 0 ? <Badge size="1" variant="soft">+{row.extraPortCount}</Badge> : null}
              </Flex>
            </Flex>
          </div>
                );
      ...
      {rowCount === 0 ? (
        <Text size="1" color="gray" style={{ padding: "var(--space-2)" }}>No matching sessions</Text>
      ) : null}
      </Flex>
    </Flex>
  );
