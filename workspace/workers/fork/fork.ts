...
async function callDO<T = unknown>(rpc: RpcCaller, ref: DORef, method: string, ...args: unknown[]): Promise<T> {
...
async function channelRef(runtime: ForkRuntime, key: string): Promise<DORef> {
    const service = await runtime.callMain<DORef & {
        targetId?: string;
    }>("workers.resolveService", CHANNEL_SERVICE_PROTOCOL, key);
    return {
...
export async function fork(runtime: ForkRuntime, opts: ForkOpts): Promise<ForkResult> {
    ...
    if (!contextId)
        throw new Error(`Channel ${opts.channelId} has no contextId`);
    ...
    for (const r of preflightResults) {
        if (!r.ok)
            throw new Error(`Cannot fork participant ${r.participantId}: ${r.reason}`);
    }
    ...
    try {
        ...
        for (const ref of clonedRefs) {
            try {
                await runtime.callMain("workerd.destroyDO", ref);
            }
            catch (e) {
                console.error(`[fork] Rollback destroy failed for ${ref.objectKey}:`, e);
            }
        }
        ...
        for (const { doRef } of toReplace.filter((_, i) => i < replacedParticipants.length)) {
            try {
                await callDO(rpc, doRef, "unsubscribeChannel", forkedChannelId);
            }
            catch (e) {
