...
import type { SendOpts, SubscribeResult, ChannelConfig, PresencePayload, StoredAttachment, } from "./types.js";
import { broadcast, broadcastConfigUpdate, sendReady, buildChannelEvent, parseRowToChannelEvent, channelEventToWsJson, queueEmit, type BroadcastDeps, cleanupDeliveryChain, } from "./broadcast.js";
...
export class PubSubChannel extends DurableObjectBase {
    ...
    private getSenderMetadata(participantId: string): Record<string, unknown> | undefined {
        const row = this.sql.exec(`SELECT metadata FROM participants WHERE id = ?`, participantId).toArray();
        if (row.length === 0)
            return undefined;
        try {
    ...
    // ── Presence events ─────────────────────────────────────────────────────
    private publishPresenceEvent(senderId: string, action: "join" | "leave" | "update", metadata: Record<string, unknown>, leaveReason?: "graceful" | "disconnect" | "replaced", senderRef?: number, persist = true): void {
        const ts = Date.now();
        ...
        if (persist) {
            this.sql.exec(`INSERT INTO messages (message_id, type, content, sender_id, ts, persist, sender_metadata)
         VALUES (?, 'presence', ?, ?, ?, 1, ?)`, messageId, payloadJson, senderId, ts, metadata ? JSON.stringify(metadata) : null);
     ...
     */
    async subscribe(participantId: string, metadata: Record<string, unknown>): Promise<SubscribeResult> {
        ...
        if (handle) {
            const conflict = this.sql.exec(`SELECT id FROM participants WHERE handle = ? AND id != ?`, handle, participantId).toArray();
            if (conflict.length > 0) {
                const otherId = conflict[0]!["id"] as string;
                throw new Error(`Participant handle "${handle}" is already in use by another participant ` +
        ...
        if (Array.isArray(advertisedMethods)) {
            ...
            for (const m of advertisedMethods) {
                const name = (m && typeof m === "object" && typeof (m as {
                    name?: unknown;
                }).name === "string")
                    ? (m as {
                        name: string;
                    ...
                    : null;
                if (name === null)
                    continue; // unknown shape; let downstream handle it
                if (!VALID_METHOD_NAME.test(name) || RESERVED_METHOD_NAMES.has(name)) {
                    throw new Error(`Invalid method name "${name}" advertised by participant "${participantId}". ` +
        ...
        const existing = this.sql.exec(`SELECT session_id FROM participants WHERE id = ?`, participantId).toArray();
        ...
        if (existing.length > 0) {
            ...
            if (sessionReplaced) {
                const pendingCountRow = this.sql.exec(`SELECT COUNT(*) as cnt FROM pending_calls WHERE target_id = ?`, participantId).toArray();
        ...
        this.sql.exec(`INSERT INTO participants (id, metadata, transport, connected_at, session_id, do_source, do_class, do_key, handle)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, participantId, JSON.stringify(storedMetadata), transport === "do" ? "do" : "rpc", Date.now(), participantSessionId, doSource ?? null, doClass ?? null, doKey ?? null, handle);
        ...
        if (wantsReplay) {
            const replayRows = this.sql.exec(`SELECT id, message_id, type, content, sender_id, ts, sender_metadata, attachments
         FROM messages WHERE type != 'presence' AND persist = 1 ORDER BY id DESC LIMIT ?`, REPLAY_LIMIT + 1).toArray();
    ...
    private sendRosterReplay(subscriberId: string): Array<Record<string, unknown>> {
        ...
        const rows = this.sql.exec(`SELECT id, message_id, type, content, sender_id, ts, sender_metadata FROM messages WHERE type = 'presence' ORDER BY id ASC`).toArray();
        ...
        for (const p of participants) {
            ...
            const event = buildChannelEvent(0, `snapshot_${pid}`, "presence", JSON.stringify(payload), pid, metadata, ts, false);
            const msg = channelEventToWsJson(event, "replay");
            replayMessages.push(msg);
            void queueEmit(this.broadcastDeps, subscriberId, {
                channelId: this.objectKey,
    ...
    private sendMessageReplay(subscriberId: string, sinceId?: number, replayMessageLimit?: number): Array<Record<string, unknown>> {
        ...
        if (sinceId && sinceId > 0) {
            rows = this.sql.exec(`SELECT id, message_id, type, content, sender_id, ts, sender_metadata, attachments
         FROM messages WHERE id > ? AND type != 'presence' ORDER BY id ASC`, sinceId).toArray();
        }
        else if (replayMessageLimit && replayMessageLimit > 0) {
            // Anchored replay: find the Nth-from-last "message" type row
            const anchorRows = this.sql.exec(`SELECT id FROM messages WHERE type = 'message' ORDER BY id DESC LIMIT 1 OFFSET ?`, replayMessageLimit - 1).toArray();
            if (anchorRows.length > 0) {
                const anchorId = anchorRows[0]!["id"] as number;
                rows = this.sql.exec(`SELECT id, message_id, type, content, sender_id, ts, sender_metadata, attachments
           FROM messages WHERE id > ? AND type != 'presence' ORDER BY id ASC`, anchorId - 1).toArray();
            }
            else {
                rows = this.sql.exec(`SELECT id, message_id, type, content, sender_id, ts, sender_metadata, attachments
           FROM messages WHERE type != 'presence' ORDER BY id ASC`).toArray();
            }
        }
        else {
    ...
    private redeliverPendingCallsTo(participantId: string): void {
        const rows = this.sql.exec(`SELECT call_id, caller_id, method, args FROM pending_calls WHERE target_id = ?`, participantId).toArray();
        if (rows.length === 0)
            return;
        const onFatal = (err: {
            code?: string;
     ...
     */
    private async failPendingCallsTargeting(targetId: string, reason: "graceful" | "disconnect" | "replaced"): Promise<void> {
    ...
    async touch(participantId: string): Promise<void> {
        this.sql.exec(`UPDATE participants SET connected_at = ? WHERE id = ?`, Date.now(), participantId);
    }
    /**
     * Send a new message (from any participant).
     */
    async send(participantId: string, messageId: string, content: string, opts?: SendOpts): Promise<void> {
        // Phase 0B: Idempotency check
        const idempotencyKey = opts?.idempotencyKey;
        if (idempotencyKey) {
            const existing = this.sql.exec(`SELECT result_id FROM dedup_keys WHERE key = ?`, idempotencyKey).toArray();
            if (existing.length > 0) {
                return;
            }
        }
        ...
        const payloadJson = JSON.stringify(payload);
        if (persist) {
            this.sql.exec(`INSERT INTO messages (message_id, type, content, sender_id, ts, persist, content_type, reply_to, sender_metadata)
         VALUES (?, 'message', ?, ?, ?, 1, ?, ?, ?)`, messageId, payloadJson, participantId, ts, contentType ?? null, replyTo ?? null, senderMetadata ? JSON.stringify(senderMetadata) : null);
            const id = this.sql.exec(`SELECT last_insert_rowid() as id`).one()["id"] as number;
            if (idempotencyKey) {
                this.sql.exec(`INSERT OR IGNORE INTO dedup_keys (key, result_id, created_at) VALUES (?, ?, ?)`, idempotencyKey, id, Date.now());
            ...
            if (idempotencyKey) {
                this.sql.exec(`INSERT OR IGNORE INTO dedup_keys (key, result_id, created_at) VALUES (?, ?, ?)`, idempotencyKey, null, Date.now());
                this.scheduleDedupCleanup();
            }
     ...
     */
    async publish(participantId: string, type: string, payload: unknown, opts?: {
        persist?: boolean;
        ref?: number;
        ...
        if (type === "method-result" && payload && typeof payload === "object") {
            ...
            if (callId && isComplete) {
                const pending = this.sql.exec(`SELECT call_id FROM pending_calls WHERE call_id = ?`, callId).toArray();
                if (pending.length > 0) {
        ...
        if (persist) {
            this.sql.exec(`INSERT INTO messages (message_id, type, content, sender_id, ts, persist, sender_metadata, attachments)
         VALUES (?, ?, ?, ?, ?, 1, ?, ?)`, messageId, type, payloadJson, participantId, ts, senderMetadata ? JSON.stringify(senderMetadata) : null, attachments ? JSON.stringify(attachments) : null);
            const id = this.sql.exec(`SELECT last_insert_rowid() as id`).one()["id"] as number;
            if (idempotencyKey) {
                this.sql.exec(`INSERT OR IGNORE INTO dedup_keys (key, result_id, created_at) VALUES (?, ?, ?)`, idempotencyKey, id, Date.now());
                this.scheduleDedupCleanup();
    ...
    async update(participantId: string, messageId: string, content: string, idempotencyKey?: string, opts?: {
        ...
        const payloadJson = JSON.stringify(payload);
        this.sql.exec(`INSERT INTO messages (message_id, type, content, sender_id, ts, persist, sender_metadata)
       VALUES (?, 'update-message', ?, ?, ?, 1, ?)`, messageId, payloadJson, participantId, ts, senderMetadata ? JSON.stringify(senderMetadata) : null);
        const id = this.sql.exec(`SELECT last_insert_rowid() as id`).one()["id"] as number;
        if (idempotencyKey) {
            this.sql.exec(`INSERT OR IGNORE INTO dedup_keys (key, result_id, created_at) VALUES (?, ?, ?)`, idempotencyKey, id, Date.now());
            this.scheduleDedupCleanup();
     ...
     */
    async error(participantId: string, messageId: string, errorMessage: string, code?: string): Promise<void> {
        const ts = Date.now();
        const senderMetadata = this.getSenderMetadata(participantId);
        ...
        const payloadJson = JSON.stringify(payload);
        this.sql.exec(`INSERT INTO messages (message_id, type, content, sender_id, ts, persist, sender_metadata)
       VALUES (?, 'error', ?, ?, ?, 1, ?)`, messageId, payloadJson, participantId, ts, senderMetadata ? JSON.stringify(senderMetadata) : null);
        const id = this.sql.exec(`SELECT last_insert_rowid() as id`).one()["id"] as number;
    ...
    async getEventRange(fromSeq: number, toSeq: number): Promise<ChannelEvent[]> {
        const rows = this.sql.exec(`SELECT id, message_id, type, content, sender_id, ts, sender_metadata, attachments
       FROM messages WHERE id > ? AND id <= ? AND persist = 1 ORDER BY id ASC`, fromSeq, toSeq).toArray();
    ...
    async updateMetadata(participantId: string, metadata: Record<string, unknown>): Promise<void> {
        this.sql.exec(`UPDATE participants SET metadata = ? WHERE id = ?`, JSON.stringify(metadata), participantId);
    ...
    async setTypingState(participantId: string, typing: boolean): Promise<void> {
        const rows = this.sql.exec(`SELECT metadata FROM participants WHERE id = ?`, participantId).toArray();
        if (rows.length === 0)
            return;
        const final = { ...JSON.parse(rows[0]!["metadata"] as string), typing };
        this.sql.exec(`UPDATE participants SET metadata = ? WHERE id = ?`, JSON.stringify(final), participantId);
    ...
    async getParticipants(): Promise<Array<{
        ...
        const rows = this.sql.exec(`SELECT id, metadata, transport, do_source, do_class, do_key FROM participants`).toArray();
        return rows.map(row => {
            const entry: {
                participantId: string;
                metadata: Record<string, unknown>;
    ...
    async callMethod(callerPid: string, targetPid: string, callId: string, method: string, args: unknown): Promise<void> {
        ...
        // Deliver to target
        const target = this.sql.exec(`SELECT transport, do_source, do_class, do_key FROM participants WHERE id = ?`, targetPid).toArray();
        if (target.length === 0) {
        ...
        if (t["transport"] === "do") {
            ...
            try {
                const result = await this.rpc.call(targetPid, "onMethodCall", [this.objectKey,
                    callId,
                    method,
                    args]);
    ...
    async cancelMethodCall(callId: string): Promise<void> {
        ...
        const call = this.sql.exec(`SELECT target_id FROM pending_calls WHERE call_id = ?`, callId).toArray();
    ...
    private async deliverCallResult(callerId: string, callId: string, result: unknown, isError: boolean): Promise<void> {
        const caller = this.sql.exec(`SELECT transport, do_source, do_class, do_key FROM participants WHERE id = ?`, callerId).toArray();
        if (caller.length === 0)
            return;
        const c = caller[0]!;
        if (c["transport"] === "do") {
            for (let attempt = 0; attempt < 3; attempt++) {
                try {
                    await this.rpc.call(callerId, "onCallResult", [callId,
                        result,
                        isError]);
                    break; // Success
                }
        ...
        const messageId = crypto.randomUUID();
        this.sql.exec(`INSERT INTO messages (message_id, type, content, sender_id, ts, persist, sender_metadata)
       VALUES (?, 'method-result', ?, ?, ?, 1, NULL)`, messageId, payloadJson, callerId, ts);
        const id = this.sql.exec(`SELECT last_insert_rowid() as id`).one()["id"] as number;
        const event: ChannelEvent = {
    ...
    private scheduleNextAlarm(): void {
        ...
        const rpcCount = this.sql.exec(`SELECT COUNT(*) as cnt FROM participants WHERE transport = 'rpc'`).toArray();
    ...
    private scheduleDedupCleanup(): void {
        if (this.getStateValue("dedup_cleanup_at"))
            return;
    ...
    private async evictStaleParticipants(): Promise<void> {
        ...
        const stale = this.sql.exec(`SELECT id, metadata FROM participants WHERE transport = 'rpc' AND connected_at < ?`, cutoff).toArray();
        for (const row of stale) {
            ...
            catch { /* corrupted metadata, use empty default */ }
            this.sql.exec(`DELETE FROM participants WHERE id = ?`, pid);
            cleanupDeliveryChain(pid);
