 ...
 */
export function queueEmit(deps: BroadcastDeps, subscriberId: string, payload: unknown, onFatalDelivery?: (err: {
    code?: string;
}) => void): Promise<void> {
    const prev = emitChains.get(subscriberId) ?? Promise.resolve();
    const next = prev.then(() => deps.rpc.emit(subscriberId, "channel:message", payload).catch((err) => {
        onFatalDelivery?.(err as {
            code?: string;
        });
    }));
 ...
 */
export function broadcast(deps: BroadcastDeps, event: ChannelEvent, envelope: BroadcastEnvelope, senderId: string): void {
    const participants = deps.sql.exec(`SELECT id, transport, do_source, do_class, do_key FROM participants`).toArray();
    ...
    for (const p of participants) {
        ...
        if (p["transport"] === "do") {
            ...
            const next: Promise<void> = prev.then(() => deps.rpc.call(pid, "onChannelEvent", [deps.objectKey, event])
                .then(() => { })
 ...
 */
export function broadcastConfigUpdate(deps: BroadcastDeps, config: Record<string, unknown>, senderId?: string, senderRef?: number): void {
 ...
 */
export function sendReady(deps: BroadcastDeps, subscriberId: string, sql: SqlStorage, contextId: string | null, channelConfig: Record<string, unknown> | null): {
    contextId?: string;
 ...
 */
export function buildChannelEvent(id: number, messageId: string, type: string, content: string, senderId: string, senderMetadata: Record<string, unknown> | undefined, ts: number, persist: boolean, attachments?: Array<{
    id: string;
    data: string;
    mimeType: string;
    name?: string;
...
export function parseRowToChannelEvent(row: Record<string, unknown>): ChannelEvent {
    ...
    if (row["sender_metadata"]) {
        try {
            senderMetadata = JSON.parse(row["sender_metadata"] as string);
        }
        catch { /* skip */ }
    }
    ...
    return buildChannelEvent(row["id"] as number, (row["message_id"] as string) ?? "", row["type"] as string, row["content"] as string, row["sender_id"] as string, senderMetadata, row["ts"] as number, true, attachments);
}
// ── Wire encoding ────────────────────────────────────────────────────────────
/**
 ...
 */
export function channelEventToWsJson(event: ChannelEvent, kind: "replay" | "persisted" | "ephemeral", ref?: number): Record<string, unknown> {
    return {
        kind,
