...
import { Theme, Card, Flex, Heading, Text, Button, TextField, Switch, Box, Badge, Spinner, IconButton, } from "@radix-ui/themes";
...
const LIST_LABELS: Record<keyof AdBlockListConfig, {
    label: string;
    description: string;
}> = {
    ads: {
...
function AdBlockSettingsPage() {
    ...
    if (loading) {
        return (<Flex align="center" justify="center" gap="2" style={{ height: "100dvh" }}>
        <Spinner />
        <Text>Loading ad block settings...</Text>
      </Flex>);
    }
    if (error) {
        return (<Flex align="center" justify="center" direction="column" gap="3" style={{ height: "100dvh" }}>
        <Text color="red">Error: {error}</Text>
        <Button onClick={loadData}>Retry</Button>
      </Flex>);
    }
    return (<Box p="4" style={{ maxWidth: "600px", margin: "0 auto" }}>
      ...
      <Card mb="4">
        <Flex justify="between" align="center">
          ...
          <Switch checked={config?.enabled ?? false} onCheckedChange={handleToggleEnabled} disabled={isSaving}/>
        </Flex>
      </Card>
      ...
      <Card mb="4">
        ...
        <Flex direction="column" gap="3">
          {(Object.keys(LIST_LABELS) as Array<keyof AdBlockListConfig>).map((list) => (<Flex key={list} justify="between" align="center">
              <Flex direction="column">
                <Text size="2" weight="medium">{LIST_LABELS[list].label}</Text>
                <Text size="1" color="gray">{LIST_LABELS[list].description}</Text>
              </Flex>
              <Switch checked={config?.lists[list] ?? false} onCheckedChange={(checked) => handleToggleList(list, checked)} disabled={isSaving || !config?.enabled}/>
            </Flex>))}
        </Flex>
      ...
      <Card mb="4">
        ...
        <Flex direction="column" gap="2" mb="3">
          {config?.customLists.map((url) => (<Flex key={url} justify="between" align="center" gap="2">
              <Text size="2" style={{ wordBreak: "break-all", flex: 1 }}>{url}</Text>
              <IconButton variant="ghost" color="red" size="1" onClick={() => handleRemoveCustomList(url)} disabled={isSaving}>
                <Cross2Icon />
              </IconButton>
            </Flex>))}
        </Flex>
        ...
        <Flex gap="2">
          <TextField.Root placeholder="https://example.com/filters.txt" value={newListUrl} onChange={(e) => setNewListUrl(e.target.value)} style={{ flex: 1 }} disabled={isSaving || !config?.enabled}/>
          <IconButton onClick={handleAddCustomList} disabled={isSaving || !newListUrl.trim() || !config?.enabled}>
            <PlusIcon />
          </IconButton>
        </Flex>
      </Card>
      ...
      <Card>
        ...
        <Flex direction="column" gap="2" mb="3">
          {config?.whitelist.map((domain) => (<Flex key={domain} justify="between" align="center" gap="2">
              <Text size="2">{domain}</Text>
              <IconButton variant="ghost" color="red" size="1" onClick={() => handleRemoveWhitelist(domain)} disabled={isSaving}>
                <Cross2Icon />
              </IconButton>
            </Flex>))}
          {config?.whitelist.length === 0 && (<Text size="2" color="gray">No whitelisted domains</Text>)}
        </Flex>

        <Flex gap="2">
          <TextField.Root placeholder="example.com or *.example.com" value={newDomain} onChange={(e) => setNewDomain(e.target.value)} onKeyDown={(e) => {
            if (e.key === "Enter" && newDomain.trim()) {
                handleAddWhitelist();
            }
        }} style={{ flex: 1 }} disabled={isSaving}/>
