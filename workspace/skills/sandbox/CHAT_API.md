...
transcript, so use `chat.send` for visible messages.) The message appears in the
conversation, is treated as panel/user intent, and can start a normal agent turn.
IDs are generated by the channel client.
