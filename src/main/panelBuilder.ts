...
export class PanelBuilder {
  ...
  private ensureRuntimeDir(panelPath: string): string {
    ...
    // Create natstack.d.ts for TypeScript support at panel root
    this.ensureProcessTypes(panelPath);

    return runtimeDir;
  }

  private ensureProcessTypes(panelPath: string): void {
    // Place the type definition at the panel root so TypeScript picks it up automatically
    const processTypesPath = path.join(panelPath, "natstack.d.ts");
    const processTypesContent = `// Type definitions for NatStack panel runtime
// This file is auto-generated by NatStack - do not edit manually
...
`;
    ...
    if (!fs.existsSync(processTypesPath) || fs.readFileSync(processTypesPath, "utf-8") !== processTypesContent) {
      fs.writeFileSync(processTypesPath, processTypesContent);
    }
  }
