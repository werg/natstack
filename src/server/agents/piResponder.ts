/**
 * Pi Responder — in-process agent service.
 *
...
import { PI_PARAMETERS } from "@natstack/agentic-messaging/config";
import { z } from "zod";
import {
  createAgentSession,
...
export class PiResponder implements AgentService {
  // Injected context (set in start())
  private ctx!: AgentServiceContext;
   ...
   * Set from ctx.contextFolderPath in start() (fail fast if missing).
   */
  ...
  /** Active Pi session (disposed on stop) */
  private piSession: AgentSession | null = null;

  // piSessionFile is persisted via ctx.state for recovery across restarts
  ...
  getConnectOptions() {
    ...
    return {
      ...
      type: "pi" as const,
      reconnect: true,
      replaySinceId: undefined, // adapter manages checkpoints
      ...(contextId && { contextId }),
      extraMetadata: {
        agentTypeId: this.ctx.agentId,
      },
      methods: {
...
Examples: "Debug React Hooks", "Refactor Auth Module", "Setup CI Pipeline"`,
  ...
  async start(ctx: AgentServiceContext): Promise<void> {
    ...
    this.queue = createMessageQueue<IncomingNewMessage>({
      ...
      onHeartbeat: async () => {
        try {
          await this.client.publish("agent-heartbeat", { agentId: this.ctx.agentId }, { persist: false });
        } catch (err) {
    ...
    this.settingsMgr = createSettingsManager<PiSettings>({
      ...
      initConfig: {
        ...
        thinkingLevel: ctx.config["thinkingLevel"] as number | undefined,
        autonomyLevel: ctx.config["autonomyLevel"] as number | undefined,
      },
    });
    await this.settingsMgr.load();

    // Fail fast if contextFolderPath is not available
    const contextFolderPath = ctx.contextFolderPath;
    ...
    this.log.info("Pi agent started");
  }

  async onEvent(event: EventStreamItem): Promise<void> {
    if ((event as { type: string }).type !== "message") return;

    const msgEvent = event as unknown as IncomingNewMessage;
    ...
    const sender = this.client.roster[msgEvent.senderId];

    // Only respond to messages from panels
    if (sender?.metadata.type === "panel" && msgEvent.senderId !== this.client.clientId) {
  ...
  async stop(): Promise<void> {
    ...
    this.log.info("Pi agent stopped");
  }
  ...
  private getOAuthProviderId(modelString: string): string | undefined {
    ...
    if (resolved && PiResponder.OAUTH_PROVIDERS.has(resolved)) return resolved;
    return undefined;
  }
  ...
  private selectDefaultModel(): string {
    ...
    for (const provider of PiResponder.PREFERRED_PROVIDERS) {
      try {
...
`;
  ...
  private async ensurePiSession(
    ...
    if (this.piSession) {
      try {
        ...
        if (this.piSession.sessionFile) {
          this.ctx.state.set({ piSessionFile: this.piSession.sessionFile });
        }
        this.piSession.dispose();
      } catch (err) {
    ...
    const persistedPiState = this.ctx.state.get<{ piSessionFile?: string }>();
    const sessionManager = persistedPiState.piSessionFile
      ? SessionManager.open(persistedPiState.piSessionFile)
    ...
    // Persist session file for resumption across restarts
    if (session.sessionFile) {
      this.ctx.state.set({ piSessionFile: session.sessionFile });
    }
  ...
  private buildModelPickerData(): {
    ...
    const preferred = PiResponder.PREFERRED_PROVIDERS;
...
`;
  ...
  private async handleUserMessage(incoming: IncomingNewMessage): Promise<void> {
    ...
    // Start pause event monitoring — controller auto-pauses + aborts; additional callback aborts Pi session
    void this.interrupt.startMonitoring(incoming.id, async () => {
      if (this.piSession) {
    ...
    try {
      ...
      this.log.error("Error processing message:", err instanceof Error ? err.message : err);
  ...
  private async handlePiEvent(
    ...
    switch (eventType) {
      ...
      case "tool_execution_end": {
        ...
        if (!ctx.interrupt.isPaused() && this.queue.getPendingCount() > 0) {
