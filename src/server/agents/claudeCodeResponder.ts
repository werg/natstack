/**
 * Claude Code Responder — in-process agent service.
 *
 * AI-powered code assistant using the Claude Agent SDK.
 * Migrated from workspace/agents/claude-code-responder to use:
 * - AgentService interface from agentAdapter
...
/**
 * Agent configuration passed at spawn time via agents.spawn RPC.
 */
...
export class ClaudeCodeResponder implements AgentService {
  ...
  // Session ID is persisted via ctx.state for recovery across restarts

  getConnectOptions() {
    const config = this.ctx.config as unknown as AgentConfig;
  ...
  async start(ctx: AgentServiceContext): Promise<void> {
    ...
    this.queue = createMessageQueue<IncomingNewMessage>({
      ...
      onHeartbeat: async () => {
        try {
          await this.client.publish("agent-heartbeat", { agentId: ctx.agentId }, { persist: false });
        } catch (err) {
    ...
    this.subagents = new SubagentManager({
      serverUrl: ctx.pubsubUrl,
      token: ctx.pubsubToken,
      channel: ctx.channel,
      parentClient: this.client,
      log: (msg) => this.log.debug(msg),
    });
    ...
    const persistedState = ctx.state.get<{ sdkSessionId?: string }>();
    if (persistedState.sdkSessionId) {
      await this.performSessionRecovery(persistedState.sdkSessionId, ctx.contextFolderPath);
    }
  ...
  private async handleUserMessage(incoming: IncomingNewMessage): Promise<void> {
    ...
    try {
      // Start pause event monitoring — controller auto-pauses + aborts; additional callback interrupts SDK query
      ...
      // Resume session (within agent lifetime only)
      const resumeSessionId = client.sdkSessionId || this.ctx.state.get<{ sdkSessionId?: string }>().sdkSessionId;
      if (resumeSessionId) {
      ...
      const canUseTool: CanUseTool = async (toolName, input, options) => {
        const result = await this.handleToolPermission(toolName, input, options, client, panel, settings, approvalGate);
      ...
      const queryOptions: Parameters<typeof query>[0]["options"] = {
        ...
        disallowedTools: [],
        cwd: this.ctx.contextFolderPath,
        ...
        canUseTool,
        // Context folder has .claude-plugin/plugin.json (generated by contextFolderManager)
        // and skills/ copied from the workspace. Point plugin at the context folder itself.
        plugins: [{ type: 'local' as const, path: this.ctx.contextFolderPath! }],
      };
        ...
        for await (const message of queryInstance) {
          ...
          if (message.type === "stream_event") {
            ...
            if (streamEvent.type === "message_start") {
              ...
              if (!this.interrupt.isPaused() && this.queue.getPendingCount() > 0 && capturedSessionId) {
      ...
      // Persist session ID for recovery across restarts
      if (capturedSessionId) {
        this.ctx.state.set({ sdkSessionId: capturedSessionId });
      ...
      // Persist session ID on error for recovery across restarts
      if (capturedSessionId) {
        this.ctx.state.set({ sdkSessionId: capturedSessionId });
        if (client.sessionKey) {
