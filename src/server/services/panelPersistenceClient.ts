...
export function createPanelPersistenceClient(rpc: RpcLike): PanelPersistence & PanelSearchIndex {
  ...
  return {
    ...
    pushHistorySnapshot: (panelId: string, snapshot) =>
      call("pushHistorySnapshot", panelId, snapshot),
    navigateHistory: (panelId: string, delta: -1 | 1) => call("navigateHistory", panelId, delta),
    setSelectedChild: (panelId: string, childId: string | null) =>
      call("setSelectedChild", panelId, childId),
    ...
    movePanel: (panelId: string, newParentId: string | null, targetPosition: number) =>
      call("movePanel", panelId, newParentId, targetPosition),
    getChildrenPaginated: (parentId: string, offset: number, limit: number) =>
      call("getChildrenPaginated", parentId, offset, limit),
    getRootPanelsPaginated: (offset: number, limit: number) =>
      call("getRootPanelsPaginated", offset, limit),
    ...
    setCollapsedBatch: (panelIds: string[], collapsed: boolean) =>
      call("setCollapsedBatch", panelIds, collapsed),
