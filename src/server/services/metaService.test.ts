...
describe("metaService", () => {
  ...
  it("lists registered services", async () => {
    const result = (await dispatcher.dispatch(ctx, "meta", "listServices", [])) as Array<{
      name: string;
    }>;
  ...
  it("returns the checked-in runtime surfaces", async () => {
    await expect(dispatcher.dispatch(ctx, "meta", "getRuntimeSurface", ["panel"])).resolves.toEqual(
      panelRuntimeSurface
    );
